...
Dự án này bao gồm cả **Giao diện Người dùng (User Interface)** phong phú dành cho khách hàng và **Bảng điều khiển Quản trị (Admin Dashboard)** mạnh mẽ để quản lý cửa hàng. Ứng dụng được tích hợp sâu với **Cloudinary** để lưu trữ hình ảnh, **Gemini AI** làm trợ lý ảo trực tiếp, và **MoMo Sandbox** để xử lý thanh toán ví điện tử.
...
- **Trợ lý AI (Google Gemini):** Tích hợp AI Chatbot làm trợ lý ảo trực tiếp trong app, hỗ trợ giải đáp câu hỏi về size giày, quần áo, sản phẩm, và chính sách cửa hàng nhanh chóng.
...
- **Thanh toán trực tuyến:** Tích hợp thanh toán qua **Ví điện tử MoMo** (Môi trường Sandbox) thông qua WebView, song song với hình thức thanh toán khi nhận hàng (COD).
- **Quản lý đơn hàng:** Theo dõi trạng thái đơn hàng (timeline chi tiết tiến độ giao hàng), xem lịch sử mua hàng, và gửi yêu cầu hủy đơn hoặc trả hàng.
- **Hồ sơ cá nhân:** Quản lý nhiều địa chỉ giao hàng, phương thức thanh toán, và ví voucher cá nhân.
- **Chế độ Sáng/Tối (Light/Dark Mode):** Tự động thay đổi giao diện mượt mà theo hệ thống hoặc thiết lập chủ động trong ứng dụng, với bộ màu sắc (AppColors) hỗ trợ tối ưu trải nghiệm nhìn.
...
- **Cấu hình ứng dụng:** Bật/tắt chế độ bảo trì, quản lý phương thức thanh toán, thiết lập luật giao hàng (shipping rules), quản lý Light/Dark mode mặc định, và quản lý banner hiển thị ở trang chủ.
...
- **Tích hợp khác:** Google Maps Flutter, Webview
...
```text
lib/
 ├── core/          # Tiện ích cốt lõi, cấu hình theme (Light/Dark mode), hằng số (constants)
 ├── features/      # Các module chức năng (Auth, Cart, Home, Profile, Admin, Chatbot...)
 ├── models/        # Các model dữ liệu
 ├── services/      # Các dịch vụ bên ngoài (Firebase, Cloudinary, MoMo, Gemini)
...
- API Key của Google Gemini (để chạy Chatbot)
...
4. **Cấu hình API Keys**
   - Đảm bảo thiết lập cấu hình hoặc tạo biến môi trường/hằng số cho các API (Cloudinary URL, Gemini API Key, MoMo Endpoint).
